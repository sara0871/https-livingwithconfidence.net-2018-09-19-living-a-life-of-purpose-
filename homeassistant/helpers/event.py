...
from ..const import ATTR_NOW, EVENT_STATE_CHANGED, EVENT_TIME_CHANGED, MATCH_ALL
...
def async_track_sunrise(hass, action, offset=None):
    ...
    def sunrise_automation_listener(now):
        ...
        remove = async_track_point_in_utc_time(
            ...
            get_astral_event_next(hass, "sunrise", offset=offset),
        )
        hass.async_run_job(action)
...
def async_track_time_change(
    hass, action, year=None, month=None, day=None, hour=None, minute=None, second=None
):
    """Add a listener that will fire if UTC time matches a pattern."""
    return async_track_utc_time_change(
        hass, action, year, month, day, hour, minute, second, local=True
    )
