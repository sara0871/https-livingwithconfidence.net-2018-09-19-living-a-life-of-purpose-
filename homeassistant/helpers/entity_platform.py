...
class EntityPlatform:
    ...
    def __init__(
        ...
        *,
        ...
        parallel_updates = getattr(
            platform, "PARALLEL_UPDATES", default_parallel_updates
        )

        if parallel_updates:
            self.parallel_updates = asyncio.Semaphore(parallel_updates, loop=hass.loop)
        else:
            self.parallel_updates = None
    ...
    async def _async_setup_platform(self, async_create_setup_task, tries=0):
        ...
        try:
            ...
            if self._tasks:
                ...
                if pending:
                    await asyncio.wait(pending, loop=self.hass.loop)
    ...
    async def async_reset(self):
        ...
        tasks = [self._async_remove_entity(entity_id) for entity_id in self.entities]
