...
def setup(hass, config):
    ...
    for index, doorstation_config in enumerate(config[DOMAIN][CONF_DEVICES]):
        ...
        if status[0]:
            ...
            _LOGGER.error(
                "Could not connect to DoorBird at %s: Error %s",
                device_ip,
                str(status[1]),
            )
            return False
...
def subscribe_events(hass, doorstation):
    ...
    for sensor_type in doorstation.monitored_events:
        name = "{} {}".format(doorstation.name, SENSOR_TYPES[sensor_type][0])
        ...
        _LOGGER.info("DoorBird will connect to this instance via %s", url)

        _LOGGER.info(
            "You may use the following event name for automations" ": %s_%s",
            DOMAIN,
            slug,
        )
