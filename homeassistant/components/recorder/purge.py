...
def purge_old_data(instance, purge_days, repack):
    ...
    with session_scope(session=instance.get_session()) as session:
        delete_states = session.query(States).filter(
            (States.last_updated < purge_before)
        )
        ...
        protected_states = (
            session.query(func.max(States.state_id)).group_by(States.entity_id).all()
        )

        protected_state_ids = tuple(state[0] for state in protected_states)

        if protected_state_ids:
            delete_states = delete_states.filter(
                ~States.state_id.in_(protected_state_ids)
            )

        deleted_rows = delete_states.delete(synchronize_session=False)
        _LOGGER.debug("Deleted %s states", deleted_rows)

        delete_events = session.query(Events).filter((Events.time_fired < purge_before))
        ...
        if protected_state_ids:
            ...
            if protected_event_ids:
                delete_events = delete_events.filter(
                    ~Events.event_id.in_(protected_event_ids)
                )
