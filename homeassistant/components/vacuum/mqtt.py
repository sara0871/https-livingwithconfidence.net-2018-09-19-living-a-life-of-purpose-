...
PLATFORM_SCHEMA = mqtt.MQTT_BASE_PLATFORM_SCHEMA.extend(
    {
        ...
        vol.Optional(CONF_PAYLOAD_TURN_ON, default=DEFAULT_PAYLOAD_TURN_ON): cv.string,
        vol.Optional(
...
class MqttVacuum(MqttAvailability, VacuumDevice):
    ...
    def async_added_to_hass(self):
        ...
        def message_received(topic, payload, qos):
            ...
            if topic == self._battery_level_topic and self._battery_level_template:
            ...
            if topic == self._charging_topic and self._charging_template:
                charging = self._charging_template.async_render_with_possible_json_value(
                    payload, error_value=None
                )
            ...
            if topic == self._cleaning_topic and self._cleaning_template:
                cleaning = self._cleaning_template.async_render_with_possible_json_value(
                    payload, error_value=None
                )
            ...
            if topic == self._docked_topic and self._docked_template:
                docked = self._docked_template.async_render_with_possible_json_value(
                    payload, error_value=None
                )
            ...
            if topic == self._fan_speed_topic and self._fan_speed_template:
                fan_speed = self._fan_speed_template.async_render_with_possible_json_value(
                    payload, error_value=None
                )
    ...
    def async_stop(self, **kwargs):
        ...
        mqtt.async_publish(
            self.hass, self._command_topic, self._payload_stop, self._qos, self._retain
        )
    ...
    def async_send_command(self, command, params=None, **kwargs):
        ...
        mqtt.async_publish(
            self.hass, self._send_command_topic, command, self._qos, self._retain
        )
