...
HC_HASS_TO_HOMEKIT = {STATE_OFF: 0, STATE_HEAT: 1, STATE_COOL: 2, STATE_AUTO: 3}
HC_HOMEKIT_TO_HASS = {c: s for s, c in HC_HASS_TO_HOMEKIT.items()}

SUPPORT_TEMP_RANGE = SUPPORT_TARGET_TEMPERATURE_LOW | SUPPORT_TARGET_TEMPERATURE_HIGH
...
class Thermostat(HomeAccessory):
    ...
    def set_heat_cool(self, value):
        ...
        if value in HC_HOMEKIT_TO_HASS:
            ...
            params = {ATTR_ENTITY_ID: self.entity_id, ATTR_OPERATION_MODE: hass_value}
    ...
    def set_cooling_threshold(self, value):
        ...
        _LOGGER.debug(
            "%s: Set cooling threshold temperature to %.2f°C", self.entity_id, value
        )
    ...
    def set_heating_threshold(self, value):
        ...
        _LOGGER.debug(
            "%s: Set heating threshold temperature to %.2f°C", self.entity_id, value
        )
    ...
    def set_target_temperature(self, value):
        ...
        _LOGGER.debug("%s: Set target temperature to %.2f°C", self.entity_id, value)
    ...
    def update_state(self, new_state):
        ...
        if self.support_power_state is True and new_state.state == STATE_OFF:
            self.char_target_heat_cool.set_value(HC_HASS_TO_HOMEKIT[STATE_OFF])
        elif operation_mode and operation_mode in HC_HASS_TO_HOMEKIT:
            if not self.heat_cool_flag_target_state:
                self.char_target_heat_cool.set_value(HC_HASS_TO_HOMEKIT[operation_mode])
        self.heat_cool_flag_target_state = False
