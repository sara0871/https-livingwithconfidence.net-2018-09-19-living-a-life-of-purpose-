...
class ConfiguredDevice:
    ...
    def save_data(self):
        ...
        for entity in self.config.get(CONF_BINARY_SENSORS) or []:
            ...
            _LOGGER.debug(
                "Set up sensor %s (initial state: %s)",
        ...
        device_data = {CONF_BINARY_SENSORS: sensors, CONF_SWITCHES: actuators}
...
class DiscoveredDevice:
    ...
    def sync_device_config(self):
        ...
        _LOGGER.debug(
            "%s: desired sensor config: %s",
            self.device_id,
            desired_sensor_configuration,
        )
        _LOGGER.debug(
            "%s: current sensor config: %s",
            self.device_id,
            current_sensor_configuration,
        )
        ...
        _LOGGER.debug(
            "%s: desired actuator config: %s", self.device_id, desired_actuator_config
        )
        _LOGGER.debug(
            "%s: current actuator config: %s", self.device_id, current_actuator_config
        )
        ...
        _LOGGER.debug(
            "%s: desired api endpoint: %s", self.device_id, desired_api_endpoint
        )
        _LOGGER.debug(
            "%s: current api endpoint: %s", self.device_id, current_api_endpoint
        )

        if (
            (desired_sensor_configuration != current_sensor_configuration)
            or (current_actuator_config != desired_actuator_config)
            or (current_api_endpoint != desired_api_endpoint)
        ):
...
class KonnectedView(HomeAssistantView):
    ...
    async def put(
        self, request: Request, device_id, pin_num=None, state=None
    ) -> Response:
        ...
        if entity_id is None:
            return self.json_message(
                "uninitialized sensor/actuator", status_code=HTTP_NOT_FOUND
            )
