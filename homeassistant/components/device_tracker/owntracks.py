...
def async_handle_transition_message(hass, context, message):
    ...
    if message["event"] == "enter":
        yield from _async_transition_message_enter(hass, context, message, location)
    elif message["event"] == "leave":
        yield from _async_transition_message_leave(hass, context, message, location)
    else:
        _LOGGER.error(
...
def async_handle_encrypted_message(hass, context, message):
    ...
    plaintext_payload = _decrypt_payload(
        context.secret, message["topic"], message["data"]
    )
