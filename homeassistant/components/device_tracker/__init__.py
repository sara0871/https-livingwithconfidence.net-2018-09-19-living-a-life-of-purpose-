        ...
        try:
            ...
            if hasattr(platform, "async_get_scanner"):
                ...
                setup = yield from hass.async_add_job(
                    platform.setup_scanner, hass, p_config, tracker.see, disc_info
                )
            else:
    ...
    def __init__(
        ...
        for dev in devices:
            ...
            if dev.mac and self.mac_to_dev[dev.mac] is not dev:
                _LOGGER.warning("Duplicate device MAC addresses detected %s", dev.mac)
    ...
    def async_see(
        ...
        if self.group and self.track_new:
            self.group.async_set_group(
                util.slugify(GROUP_NAME_ALL_DEVICES),
                visible=False,
    ...
    def async_setup_tracked_device(self):
        ...
        for device in self.devices.values():
            if device.track and not device.last_seen:
                tasks.append(self.hass.async_add_job(async_init_single_device(device)))
...
class Device(Entity):
    ...
    def stale(self, now: dt_util.dt.datetime = None):
        ...
        return (
            self.last_seen
            and (now or dt_util.utcnow()) - self.last_seen > self.consider_home
        )
...
def load_config(path: str, hass: HomeAssistantType, consider_home: timedelta):
    ...
    return run_coroutine_threadsafe(
        async_load_config(path, hass, consider_home), hass.loop
    ).result()


@asyncio.coroutine
def async_load_config(path: str, hass: HomeAssistantType, consider_home: timedelta):
    """Load devices from YAML configuration file.
    ...
    try:
        result = []
        try:
            devices = yield from hass.async_add_job(load_yaml_config_file, path)
