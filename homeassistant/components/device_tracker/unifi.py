...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        ...
        vol.Optional(CONF_MONITORED_CONDITIONS): vol.All(
            cv.ensure_list, [vol.In(AVAILABLE_ATTRS)]
        ),
...
def get_scanner(hass, config):
    ...
    return UnifiScanner(ctrl, detection_time, ssid_filter, monitored_conditions)
...
class UnifiScanner(DeviceScanner):
    ...
    def __init__(
        self, controller, detection_time: timedelta, ssid_filter, monitored_conditions
    ) -> None:
    ...
    def _update(self):
        ...
        if self._ssid_filter:
            clients = [
                ...
                if "essid" in client and client["essid"] in self._ssid_filter
            ]
