...
def async_setup(hass, config):
    ...
    tasks = [bootstrap.async_setup_component(hass, "sun")]

    # Set up demo platforms
    demo_config = config.copy()
    for component in COMPONENTS_WITH_DEMO_PLATFORM:
        demo_config[component] = {CONF_PLATFORM: "demo"}
        tasks.append(bootstrap.async_setup_component(hass, component, demo_config))
    ...
    tasks2.append(
        group.Group.async_create_group(
            hass,
            "Bedroom",
            [lights[0], switches[1], media_players[0], "input_number.noise_allowance"],
        )
    )
    tasks2.append(
        group.Group.async_create_group(
            hass, "Kitchen", [lights[2], "cover.kitchen_window", "lock.kitchen_door"]
        )
    )
    tasks2.append(
        group.Group.async_create_group(
    ...
    tasks2.append(
        group.Group.async_create_group(
            hass, "Automations", ["input_select.who_cooks", "input_boolean.notify"]
        )
    )
    tasks2.append(
        group.Group.async_create_group(
