...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    device = hass.data[DATA_DEVICES].pop(discovery_info[const.DISCOVERY_DEVICE], None)
    if device is None:
        return False
...
async def async_setup(hass, config):
    ...
    def node_added(node):
        ...
        hass.add_job(check_has_unique_id, entity, _on_ready, _on_timeout, hass.loop)

    def network_ready():
        """Handle the query of all awake nodes."""
        _LOGGER.info(
            "Zwave network is ready for use. All awake nodes "
    ...
    def _finalize_start():
        ...
        polling_interval = convert(config[DOMAIN].get(CONF_POLLING_INTERVAL), int)
        ...
        hass.services.register(DOMAIN, const.SERVICE_START_NETWORK, start_zwave)
...
class ZWaveDeviceEntityValues:
    ...
    def _check_entity_ready(self):
        ...
        def _on_ready(sec):
            ...
            self._hass.async_add_job(discover_device, component, device, dict_id)
        ...
        async def discover_device(component, device, dict_id):
            ...
            await discovery.async_load_platform(
                ...
                component,
                DOMAIN,
                {const.DISCOVERY_DEVICE: dict_id},
                self._zwave_config,
            )
        ...
        if device.unique_id:
            ...
            self._hass.add_job(
                check_has_unique_id, device, _on_ready, _on_timeout, self._hass.loop
            )
