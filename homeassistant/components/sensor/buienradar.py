...
def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    for sensor_type in config[CONF_MONITORED_CONDITIONS]:
        dev.append(BrSensor(sensor_type, config.get(CONF_NAME, "br"), coordinates))
...
class BrData:
    ...
    def get_data(self, url):
        ...
        from buienradar.buienradar import CONTENT, MESSAGE, STATUS_CODE, SUCCESS
    ...
    def async_update(self, *_):
        ...
        if content.get(SUCCESS) is not True:
            ...
            _LOGGER.warning(
                "Unable to retrieve xml data from Buienradar." "(Msg: %s, status: %s,)",
                content.get(MESSAGE),
