...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    if not categories:
        ...
        for category in categories:
            device = GeoRssServiceSensor(category, data, name, unit_of_measurement)
            devices.append(device)
...
class GeoRssServiceSensor(Entity):
    ...
    def update(self):
        ...
        if self._data.events is None:
            ...
            _LOGGER.debug("Adding events to sensor %s: %s", self.entity_id, my_events)
            self._state = len(my_events)
            # And now compute the attributes from the filtered events.
            matrix = {}
            for event in my_events:
                matrix[event[ATTR_TITLE]] = "{:.0f}km".format(event[ATTR_DISTANCE])
...
class GeoRssServiceData:
    ...
    def filter_entries(self, feed_data):
        ...
        events = []
        _LOGGER.debug(
            "%s entri(es) available in feed %s", len(feed_data.entries), self._url
        )
    ...
    def calculate_distance_to_polygon(self, polygon):
        ...
        for polygon_point in polygon:
            ...
            distance = min(distance, self.calculate_distance_to_coords(coordinates))
        _LOGGER.debug(
            "Distance from %s to %s: %s km", self._home_coordinates, polygon, distance
        )
        return distance
