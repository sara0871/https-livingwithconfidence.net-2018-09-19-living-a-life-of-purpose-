...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    sensor = WazeTravelTime(
        name, origin, destination, region, incl_filter, excl_filter, realtime
    )
...
class WazeTravelTime(Entity):
    ...
    def __init__(
        self, name, origin, destination, region, incl_filter, excl_filter, realtime
    ):
            ...
            try:
                ...
                if self._incl_filter is not None:
                    routes = {
                        ...
                        if self._incl_filter.lower() in k.lower()
                    }
                ...
                if self._excl_filter is not None:
                    routes = {
                        k: v
                        for k, v in routes.items()
                        if self._excl_filter.lower() not in k.lower()
                    }
