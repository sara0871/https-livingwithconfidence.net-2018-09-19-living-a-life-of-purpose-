...
SERVICE_SCAN_SCHEMA = vol.Schema({vol.Optional(ATTR_ENTITY_ID): cv.entity_ids})
...
async def async_setup(hass, config):
    ...
    async def async_scan_service(service):
        ...
        update_task = [entity.async_update_ha_state(True) for entity in image_entities]
        if update_task:
            await asyncio.wait(update_task, loop=hass.loop)

    hass.services.async_register(
        DOMAIN, SERVICE_SCAN, async_scan_service, schema=SERVICE_SCAN_SCHEMA
    )
