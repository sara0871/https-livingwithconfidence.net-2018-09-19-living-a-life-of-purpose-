...
class KodiDevice(MediaPlayerDevice):
    ...
    def async_update(self):
        ...
        self._app_properties = yield from self.server.Application.GetProperties(
            ["volume", "muted"]
        )
        ...
        if self._players:
            ...
            self._item = (
                yield from self.server.Player.GetItem(
                    player_id,
                    [
                        "title",
                        "file",
    ...
    def async_volume_down(self):
        ...
        assert (yield from self.server.Input.ExecuteAction("volumedown")) == "OK"
    ...
    def async_set_play_state(self, state):
        ...
        if players is not None and players:
            yield from self.server.Player.PlayPause(players[0]["playerid"], state)
    ...
    def _goto(self, direction):
        ...
        if players:
            ...
            yield from self.server.Player.GoTo(players[0]["playerid"], direction)
    ...
    def async_add_media_to_playlist(
        ...
        if media_type == "SONG":
            ...
            if media_id is None:
                ...
                media_id = yield from self.async_find_album(media_name, artist_name)
            if media_id:
    ...
    def async_get_albums(self, artist_id=None):
        ...
        return (
            yield from self.server.AudioLibrary.GetAlbums(
    ...
    def async_get_songs(self, artist_id=None):
        ...
        return (
            yield from self.server.AudioLibrary.GetSongs(
                {"filter": {"artistid": int(artist_id)}}
            )
        )
