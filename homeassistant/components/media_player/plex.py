...
def setup_plexserver(
    ...
    def update_devices():
        ...
        for device in devices:
            ...
            if device.machineIdentifier not in plex_clients:
                new_client = PlexClient(
                    config, device, None, plex_sessions, update_devices, update_sessions
                )
        ...
        for client in plex_clients.values():
            ...
            client.set_availability(client.machine_identifier in available_client_ids)
...
class PlexClient(MediaPlayerDevice):
    ...
    def __init__(
        self, config, device, session, plex_sessions, update_devices, update_sessions
    ):
    ...
    def refresh(self, device, session):
        ...
        if self._session:
            if self._device.machineIdentifier is not None and self._session.players:
