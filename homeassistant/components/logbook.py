...
class LogbookView(HomeAssistantView):
    ...
    async def get(self, request, datetime=None):
        ...
        def json_events():
            ...
            return self.json(list(_get_events(hass, self.config, start_day, end_day)))
...
class Entry:
    ...
    def __init__(self, when=None, name=None, message=None, domain=None, entity_id=None):
    ...
    for _, g_events in groupby(
        ...
        for event in events_batch:
            if event.event_type == EVENT_STATE_CHANGED:
                ...
                yield Entry(
                    event.time_fired, "Home Assistant", "started", domain=HA_DOMAIN
                )
                ...
                yield Entry(
                    event.time_fired, "Home Assistant", action, domain=HA_DOMAIN
                )
...
def _exclude_events(events, config):
    ...
    for event in events:
        ...
        if domain or entity_id:
            ...
            if excluded_domains and domain in excluded_domains and not included_domains:
                if (
            ...
            # filter if only included is configured for this domain
                ...
                not excluded_domains
                and included_domains
                and domain not in included_domains
            ):
                if (
                    included_entities and entity_id not in included_entities
                ) or not included_entities:
            ...
            # filter if included and excluded is configured for this domain
                ...
                excluded_domains
                and included_domains
                and (domain not in included_domains or domain in excluded_domains)
            ):
                if (
                    (included_entities and entity_id not in included_entities)
                    or not included_entities
            ...
            # filter if only included is configured for this entity
                ...
                not excluded_domains
                and not included_domains
                and included_entities
                and entity_id not in included_entities
            ):
                continue
