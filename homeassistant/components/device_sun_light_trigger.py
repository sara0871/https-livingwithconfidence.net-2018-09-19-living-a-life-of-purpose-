...
def async_setup(hass, config):
    ...
    def async_turn_on_before_sunset(light_id):
        ...
        if not device_tracker.is_on() or light.is_on(light_id):
            return
        light.async_turn_on(
            light_id, transition=LIGHT_TRANSITION_TIME.seconds, profile=light_profile
        )
    ...
    async_track_point_in_utc_time(
        hass, schedule_light_turn_on, get_astral_event_next(hass, "sunrise")
    )
    ...
    def check_light_on_dev_state_change(entity, old_state, new_state):
        ...
        elif start_point and start_point < now < get_astral_event_next(hass, "sunset"):
