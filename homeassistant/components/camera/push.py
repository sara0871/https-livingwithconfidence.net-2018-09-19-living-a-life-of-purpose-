...
from homeassistant.components.http.view import KEY_AUTHENTICATED, HomeAssistantView
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    hass.http.register_view(CameraPushReceiver(hass, config[CONF_IMAGE_FIELD]))
...
class CameraPushReceiver(HomeAssistantView):
    ...
    async def post(self, request, entity_id):
        ...
        if _camera is None:
            ...
            return self.json_message("Unknown {}".format(entity_id), status)

        # Supports HA authentication and token based
        # when token has been configured
        authenticated = request[KEY_AUTHENTICATED] or (
            _camera.token is not None and request.query.get("token") == _camera.token
        )
        ...
        try:
            ...
            await _camera.update_image(
                data[self._image].file.read(), data[self._image].filename
            )
            ...
            return self.json_message("{} missing".format(self._image), HTTP_BAD_REQUEST)
