...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    hass.services.async_register(
        DOMAIN, SERVICE_PTZ, handle_ptz, schema=SERVICE_PTZ_SCHEMA
    )
...
class ONVIFHassCamera(Camera):
    ...
    def __init__(self, hass, config):
        ...
        self._input = None
        self._media_service = onvif.ONVIFService(
            "http://{}:{}/onvif/device_service".format(self._host, self._port),
            self._username,
            self._password,
            "{}/wsdl/media.wsdl".format(os.path.dirname(onvif.__file__)),
        )

        self._ptz_service = onvif.ONVIFService(
            "http://{}:{}/onvif/device_service".format(self._host, self._port),
            self._username,
            self._password,
    ...
    def obtain_input_uri(self):
        ...
        try:
            ...
            _LOGGER.debug("Couldn't setup camera '%s'. Error: %s", self._name, err)
            return
