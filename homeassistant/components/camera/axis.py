...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    camera_config = {
        ...
        CONF_MJPEG_URL: _get_image_url(
            discovery_info[CONF_HOST], str(discovery_info[CONF_PORT]), "mjpeg"
        ),
        CONF_STILL_IMAGE_URL: _get_image_url(
            discovery_info[CONF_HOST], str(discovery_info[CONF_PORT]), "single"
        ),
        CONF_AUTHENTICATION: HTTP_DIGEST_AUTHENTICATION,
    }
    add_entities([AxisCamera(hass, camera_config, str(discovery_info[CONF_PORT]))])
