...
FAN_SET_SPEED_SCHEMA = vol.Schema(
    {vol.Required(ATTR_ENTITY_ID): cv.entity_ids, vol.Required(ATTR_SPEED): cv.string}
)  # type: dict

FAN_TURN_ON_SCHEMA = vol.Schema(
    {vol.Required(ATTR_ENTITY_ID): cv.entity_ids, vol.Optional(ATTR_SPEED): cv.string}
)  # type: dict
...
def async_setup(hass, config: dict):
    ...
    component.async_register_entity_service(
        SERVICE_TURN_OFF, FAN_TURN_OFF_SCHEMA, "async_turn_off"
    )
    component.async_register_entity_service(
        SERVICE_TOGGLE, FAN_TOGGLE_SCHEMA, "async_toggle"
    )
    component.async_register_entity_service(
