...
FEATURE_FLAGS_AIRPURIFIER_V3 = FEATURE_FLAGS_GENERIC | FEATURE_SET_LED

FEATURE_FLAGS_AIRHUMIDIFIER = (
    FEATURE_FLAGS_GENERIC | FEATURE_SET_LED_BRIGHTNESS | FEATURE_SET_TARGET_HUMIDITY
)
...
class XiaomiAirHumidifier(XiaomiGenericDevice):
    ...
    def __init__(self, name, device, model, unique_id):
        ...
        if self._model == MODEL_AIRHUMIDIFIER_CA:
            ...
            self._speed_list = [
                mode.name for mode in OperationMode if mode.name != "Auto"
            ]
