...
async def async_start(hass, discovery_topic, hass_config):
    ...
    async def async_device_message_received(topic, payload, qos):
        ...
        if platform not in ALLOWED_PLATFORMS.get(component, []):
            _LOGGER.warning(
                "Platform %s (component %s) is not allowed", platform, component
            )
            return
        ...
        if discovery_hash in hass.data[ALREADY_DISCOVERED]:
            _LOGGER.info(
                "Component has already been discovered: %s %s", component, discovery_id
            )
            return
        ...
        await async_load_platform(hass, component, platform, payload, hass_config)
