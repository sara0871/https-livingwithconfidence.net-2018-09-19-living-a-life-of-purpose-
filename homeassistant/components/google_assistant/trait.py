    ...
    async def execute(self, command, params):
        ...
        if domain == light.DOMAIN:
            ...
            await self.hass.services.async_call(
                ...
                {
                    ATTR_ENTITY_ID: self.state.entity_id,
                    media_player.ATTR_MEDIA_VOLUME_LEVEL: params["brightness"] / 100,
                },
                blocking=True,
            )
...
class ColorSpectrumTrait(_Trait):
    ...
    def query_attributes(self):
        ...
        if color_hs is not None:
            response["color"] = {
                "spectrumRGB": int(
                    color_util.color_rgb_to_hex(*color_util.color_hs_to_RGB(*color_hs)),
                    16,
                )
            }
    ...
    def can_execute(self, command, params):
        """Test if command can be executed."""
        return command in self.commands and "spectrumRGB" in params.get("color", {})
...
class ColorTemperatureTrait(_Trait):
    ...
    def query_attributes(self):
        ...
        if temp is not None:
            response["color"] = {
                "temperature": color_util.color_temperature_mired_to_kelvin(temp)
            }
    ...
    def can_execute(self, command, params):
        """Test if command can be executed."""
        return command in self.commands and "temperature" in params.get("color", {})
    ...
    async def execute(self, command, params):
        ...
        if temp < min_temp or temp > max_temp:
            raise SmartHomeError(
                ...
                "Temperature should be between {} and {}".format(min_temp, max_temp),
            )
...
class TemperatureSettingTrait(_Trait):
    ...
    def query_attributes(self):
        ...
        if (
            operation == climate.STATE_AUTO
            and climate.ATTR_TARGET_TEMP_HIGH in attrs
            and climate.ATTR_TARGET_TEMP_LOW in attrs
        ):
    ...
    async def execute(self, command, params):
        ...
        if command == COMMAND_THERMOSTAT_TEMPERATURE_SETPOINT:
            ...
            await self.hass.services.async_call(
                ...
                {ATTR_ENTITY_ID: self.state.entity_id, climate.ATTR_TEMPERATURE: temp},
                blocking=True,
            )
            ...
            await self.hass.services.async_call(
                ...
                {
                    ATTR_ENTITY_ID: self.state.entity_id,
                    climate.ATTR_OPERATION_MODE: self.google_to_hass[
                        params["thermostatMode"]
                    ],
                },
                blocking=True,
            )
