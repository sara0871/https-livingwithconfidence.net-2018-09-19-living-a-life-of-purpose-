...
from homeassistant.components.google import CONF_OFFSET, CONF_DEVICE_ID, CONF_NAME
...
class CalendarEventDevice(Entity):
    ...
    def __init__(self, hass, data):
        ...
        self.entity_id = generate_entity_id(ENTITY_ID_FORMAT, self.dev_id, hass=hass)
    ...
    def offset_reached(self):
        ...
        if (
            self._cal_data["start"] is None
            or self._cal_data["offset_time"] == dt.dt.timedelta()
        ):
            return False

        return self._cal_data["start"] + self._cal_data["offset_time"] <= dt.now(
            self._cal_data["start"].tzinfo
        )
