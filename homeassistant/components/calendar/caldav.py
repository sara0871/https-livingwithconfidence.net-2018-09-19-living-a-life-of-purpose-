...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        ...
        vol.Optional(CONF_CALENDARS, default=[]): vol.All(
            cv.ensure_list, vol.Schema([cv.string])
        ),
...
def setup_platform(hass, config, add_entities, disc_info=None):
    ...
    for calendar in list(calendars):
        ...
        if config.get(CONF_CALENDARS) and calendar.name not in config.get(
            CONF_CALENDARS
        ):
        ...
        for cust_calendar in config.get(CONF_CUSTOM_CALENDARS):
            ...
            calendar_devices.append(
                WebDavCalendarEventDevice(
                    hass, device_data, calendar, True, cust_calendar.get(CONF_SEARCH)
                )
            )

        # Create a default calendar if there was no custom one
        if not config.get(CONF_CUSTOM_CALENDARS):
            device_data = {CONF_NAME: calendar.name, CONF_DEVICE_ID: calendar.name}
            calendar_devices.append(
...
class WebDavCalendarEventDevice(CalendarEventDevice):
    ...
    def __init__(self, hass, device_data, calendar, all_day=False, search=None):
