...
CONFIG_SCHEMA = vol.Schema(
    {DOMAIN: vol.Schema({cv.slug: SCRIPT_ENTRY_SCHEMA})}, extra=vol.ALLOW_EXTRA
)

SCRIPT_SERVICE_SCHEMA = vol.Schema(dict)
SCRIPT_TURN_ONOFF_SCHEMA = vol.Schema(
    {vol.Optional(ATTR_ENTITY_ID): cv.entity_ids, vol.Optional(ATTR_VARIABLES): dict}
)
...
class ScriptEntity(ToggleEntity):
    ...
    async def async_turn_on(self, **kwargs):
        """Turn the script on."""
        await self.script.async_run(kwargs.get(ATTR_VARIABLES), kwargs.get("context"))
