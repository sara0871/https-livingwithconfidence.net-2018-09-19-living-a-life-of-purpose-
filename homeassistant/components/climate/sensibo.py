...
def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    devices = []
    try:
        for dev in (yield from client.async_get_devices(_INITIAL_FETCH_FIELDS)):
    ...
    if devices:
        ...
        def async_assume_state(service):
            ...
            for climate in target_climate:
                yield from climate.async_assume_state(service.data.get(ATTR_STATE))
        ...
        hass.services.async_register(
            DOMAIN, SERVICE_ASSUME_STATE, async_assume_state, schema=ASSUME_STATE_SCHEMA
        )
...
class SensiboClimate(ClimateDevice):
    ...
    def _do_update(self, data):
        ...
        self._current_capabilities = capabilities["modes"][self.current_operation]
    ...
    def async_assume_state(self, state):
        """Set external state."""
        change_needed = (state != STATE_OFF and not self.is_on) or (
            state == STATE_OFF and self.is_on
        )
        if change_needed:
            with async_timeout.timeout(TIMEOUT):
                yield from self._client.async_set_ac_state_property(
    ...
    def async_update(self):
        ...
        try:
            with async_timeout.timeout(TIMEOUT):
                data = yield from self._client.async_get_device(self._id, _FETCH_FIELDS)
                self._do_update(data)
