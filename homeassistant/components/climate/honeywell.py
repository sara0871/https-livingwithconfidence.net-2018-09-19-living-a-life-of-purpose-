...
def _setup_round(username, password, config, add_entities):
    ...
    try:
        ...
        for i, zone in enumerate(zones):
            add_entities(
                [RoundThermostat(evo_api, zone["id"], i == 0, away_temp)], True
            )
    except socket.error:
        _LOGGER.error("Connection error logging into the honeywell evohome web service")
        return False
    return True
...
class RoundThermostat(ClimateDevice):
    ...
    def update(self):
        ...
        try:
            ...
            _LOGGER.error(
                "Did not receive any temperature data from the " "evohomeclient API"
            )
            return
        ...
        if data["thermostat"] == "DOMESTIC_HOT_WATER":
            ...
            self.client.system_mode = device["thermostat"]["changeableValues"]["mode"]
