...
SCHEMA_PING = BASE_COMMAND_MESSAGE_SCHEMA.extend({vol.Required("type"): TYPE_PING})
...
def event_message(iden, event):
    ...
    return {"id": iden, "type": TYPE_EVENT, "event": event.as_dict()}
...
def pong_message(iden):
    ...
    return {"id": iden, "type": TYPE_PONG}


def result_message(iden, result=None):
    """Return a success result message."""
    return {"id": iden, "type": TYPE_RESULT, "success": True, "result": result}
...
async def async_setup(hass, config):
    ...
    async_register_command(
        hass, TYPE_SUBSCRIBE_EVENTS, handle_subscribe_events, SCHEMA_SUBSCRIBE_EVENTS
    )
    async_register_command(
        hass,
    ...
    async_register_command(
        hass, TYPE_CALL_SERVICE, handle_call_service, SCHEMA_CALL_SERVICE
    )
    async_register_command(hass, TYPE_GET_STATES, handle_get_states, SCHEMA_GET_STATES)
    async_register_command(
        hass, TYPE_GET_SERVICES, handle_get_services, SCHEMA_GET_SERVICES
    )
    async_register_command(hass, TYPE_GET_CONFIG, handle_get_config, SCHEMA_GET_CONFIG)
    async_register_command(hass, TYPE_PING, handle_ping, SCHEMA_PING)
...
class ActiveConnection:
    ...
    def send_message_outside(self, message):
        ...
        try:
            ...
            self.log_error("Client exceeded max pending messages [2]:", MAX_PENDING_MSG)
            self.cancel()
        ...
        try:
            ...
            if self.hass.auth.active or not authenticated:
                ...
                if self.hass.auth.active and "access_token" in msg:
                    ...
                    not self.hass.auth.active or self.hass.auth.support_legacy
                ) and "api_password" in msg:
                    self.debug("Received api_password")
                    authenticated = validate_password(request, msg["api_password"])
            ...
            while msg:
                ...
                if cur_id <= last_id:
                    self.to_write.put_nowait(
                        error_message(
                            cur_id, ERR_ID_REUSE, "Identifier values have to increase."
                        )
                    )

                elif msg["type"] not in handlers:
                    self.log_error("Received invalid command: {}".format(msg["type"]))
                    self.to_write.put_nowait(
                        error_message(cur_id, ERR_UNKNOWN_COMMAND, "Unknown command.")
                    )
            ...
            if not authenticated:
                ...
                final_message = error_message(iden, ERR_INVALID_FORMAT, error_msg)
            ...
            self.log_error("Client exceeded max pending messages [1]:", MAX_PENDING_MSG)
            self._writer_task.cancel()
...
def handle_unsubscribe_events(hass, connection, msg):
    ...
    if subscription in connection.event_listeners:
        ...
        connection.to_write.put_nowait(
            error_message(msg["id"], ERR_NOT_FOUND, "Subscription not found.")
        )
...
def handle_call_service(hass, connection, msg):
    ...
    async def call_service_helper(msg):
        ...
        blocking = True
        if msg["domain"] == "homeassistant" and msg["service"] in ["restart", "stop"]:
            blocking = False
...
def handle_get_states(hass, connection, msg):
    ...
    Async friendly.
    """
    connection.to_write.put_nowait(result_message(msg["id"], hass.states.async_all()))
...
def handle_get_config(hass, connection, msg):
    ...
    Async friendly.
    """
    connection.to_write.put_nowait(result_message(msg["id"], hass.config.as_dict()))
...
def ws_require_user(
    ...
    def validator(func):
        ...
        @wraps(func)
        def check_current_user(hass: HomeAssistant, connection: ActiveConnection, msg):
            """Check current user."""

            def output_error(message_id, message):
                """Output error message."""
                connection.send_message_outside(
                    error_message(msg["id"], message_id, message)
                )
