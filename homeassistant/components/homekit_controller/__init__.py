...
class HKDevice:
    ...
    def accessory_setup(self):
        ...
        self.controllerkey, self.accessorykey = homekit.get_session_keys(
            self.conn, self.pairing_data
        )
        self.securecon = homekit.SecureHttp(
            self.conn.sock, self.accessorykey, self.controllerkey
        )
        ...
        for accessory in data["accessories"]:
            ...
            aid = accessory["aid"]
            for service in accessory["services"]:
                service_info = {"serial": serial, "aid": aid, "iid": service["iid"]}
                ...
                if component is not None:
                    discovery.load_platform(
                        self.hass, component, DOMAIN, service_info, self.config
                    )
    ...
    def device_config_callback(self, callback_data):
        ...
        try:
            self.pairing_data = homekit.perform_pair_setup(self.conn, code, pairing_id)
    ...
    def configure(self):
        ...
        self.hass.data[DOMAIN + self.hkid] = self.configurator.request_config(
            self.model,
            self.device_config_callback,
            description=description,
            submit_caption="submit",
            fields=[{"id": "code", "name": "HomeKit code", "type": "string"}],
        )
...
def setup(hass, config):
    ...
    def discovery_dispatch(service, discovery_info):
        ...
        if hkid in hass.data[KNOWN_DEVICES]:
            ...
            if config_num > device.config_num and device.pairing_info is not None:
