...
class MqttLight(MqttAvailability, Light):
    ...
    def __init__(
        ...
        self._supported_features |= (
            topic[CONF_BRIGHTNESS_COMMAND_TOPIC] is not None and SUPPORT_BRIGHTNESS
        )
        self._supported_features |= (
            topic[CONF_COLOR_TEMP_COMMAND_TOPIC] is not None and SUPPORT_COLOR_TEMP
        )
        self._supported_features |= (
            topic[CONF_EFFECT_COMMAND_TOPIC] is not None and SUPPORT_EFFECT
        )
        self._supported_features |= (
            topic[CONF_WHITE_VALUE_COMMAND_TOPIC] is not None and SUPPORT_WHITE_VALUE
        )
        self._supported_features |= (
    ...
    async def async_added_to_hass(self):
        ...
        if self._topic[CONF_STATE_TOPIC] is not None:
            await mqtt.async_subscribe(
                self.hass, self._topic[CONF_STATE_TOPIC], state_received, self._qos
            )
        ...
        if self._topic[CONF_BRIGHTNESS_STATE_TOPIC] is not None:
            await mqtt.async_subscribe(
                self.hass,
                self._topic[CONF_BRIGHTNESS_STATE_TOPIC],
                brightness_received,
                self._qos,
            )
        ...
        if self._topic[CONF_RGB_STATE_TOPIC] is not None:
            await mqtt.async_subscribe(
                self.hass, self._topic[CONF_RGB_STATE_TOPIC], rgb_received, self._qos
            )
            self._hs = (0, 0)
        if (
        ...
        if self._topic[CONF_COLOR_TEMP_STATE_TOPIC] is not None:
            await mqtt.async_subscribe(
                self.hass,
                self._topic[CONF_COLOR_TEMP_STATE_TOPIC],
                color_temp_received,
                self._qos,
            )
        ...
        if self._topic[CONF_EFFECT_STATE_TOPIC] is not None:
            await mqtt.async_subscribe(
                self.hass,
                self._topic[CONF_EFFECT_STATE_TOPIC],
                effect_received,
                self._qos,
            )
            self._effect = "none"
        ...
        if self._topic[CONF_WHITE_VALUE_STATE_TOPIC] is not None:
            await mqtt.async_subscribe(
                self.hass,
                self._topic[CONF_WHITE_VALUE_STATE_TOPIC],
        ...
        def xy_received(topic, payload, qos):
            ...
            xy_color = [float(val) for val in templates[CONF_XY](payload).split(",")]
            self._hs = color_util.color_xy_to_hs(*xy_color)
            self.async_schedule_update_ha_state()

        if self._topic[CONF_XY_STATE_TOPIC] is not None:
            await mqtt.async_subscribe(
                self.hass, self._topic[CONF_XY_STATE_TOPIC], xy_received, self._qos
            )
            self._hs = (0, 0)
        if (
            self._optimistic_xy
