...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    if hasattr(endpoint, "light_color"):
        caps = await zha.safe_read(endpoint.light_color, ["color_capabilities"])
        ...
        if discovery_info["color_capabilities"] is None:
            ...
            result = await zha.safe_read(endpoint.light_color, ["color_temperature"])
...
class Light(zha.Entity, light.Light):
    ...
    async def async_turn_on(self, **kwargs):
        ...
        if light.ATTR_COLOR_TEMP in kwargs:
            ...
            await self._endpoint.light_color.move_to_color_temp(temperature, duration)
        ...
        if self._brightness is not None:
            ...
            await self._endpoint.level.move_to_level_with_on_off(brightness, duration)
            self._state = 1
    ...
    async def async_update(self):
        ...
        if self._supported_features & light.SUPPORT_COLOR_TEMP:
            ...
            self._color_temp = result.get("color_temperature", self._color_temp)
        ...
        if self._supported_features & light.SUPPORT_COLOR:
            ...
            if "current_x" in result and "current_y" in result:
                xy_color = (
                    round(result["current_x"] / 65535, 3),
