...
def request_configuration(hass, config, host):
    ...
    async def configuration_callback(callback_data):
        ...
        try:
            ...
            configurator.async_notify_errors(
                hass, instance, "Security Code not accepted."
            )
            return

        res = await _setup_gateway(
            hass, config, host, identity, key, DEFAULT_ALLOW_TRADFRI_GROUPS
        )

        if not res:
            configurator.async_notify_errors(hass, instance, "Unable to connect.")
            return
    ...
    instance = configurator.request_config(
        ...
        fields=[{"id": "security_code", "name": "Security Code", "type": "password"}],
    )
...
async def async_setup(hass, config):
    ...
    known_hosts = await hass.async_add_job(load_json, hass.config.path(CONFIG_FILE))

    async def gateway_discovered(
        service, info, allow_groups=DEFAULT_ALLOW_TRADFRI_GROUPS
    ):
        ...
        if host in known_hosts:
            ...
            await _setup_gateway(hass, config, host, identity, key, allow_groups)
        else:
...
async def _setup_gateway(hass, hass_config, host, identity, key, allow_tradfri_groups):
    """Create a gateway."""
