...
DOMAIN = "frontend"
DEPENDENCIES = ["api", "websocket_api", "http", "system_log", "auth", "onboarding"]
        ...
        DOMAIN: vol.Schema(
            {
                ...
                vol.Optional(CONF_EXTRA_HTML_URL_ES5): vol.All(
                    cv.ensure_list, [cv.string]
                ),
                vol.Optional(CONF_JS_VERSION, default=JS_DEFAULT_OPTION): vol.In(
                    JS_OPTIONS
                ),
            }
        )
...
class Panel:
    ...
    def __init__(
        self, component_name, sidebar_title, sidebar_icon, frontend_url_path, config
    ):
    ...
    def async_register_index_routes(self, router, index_view):
        ...
        router.add_route("get", "/{}".format(self.frontend_url_path), index_view.get)
        router.add_route(
...
async def async_setup(hass, config):
    ...
    hass.http.register_static_path(
        ...
        os.path.join(hass_frontend_es5_path, "service_worker.js"),
        False,
    )
    hass.http.register_static_path(
        "/service_worker.js",
        os.path.join(hass_frontend_path, "service_worker.js"),
        False,
    )
    hass.http.register_static_path(
    ...
    hass.http.register_static_path("/frontend_latest", hass_frontend_path, not is_dev)
    hass.http.register_static_path("/frontend_es5", hass_frontend_es5_path, not is_dev)
...
class AuthorizeView(HomeAssistantView):
    ...
    async def get(self, request: web.Request):
        ...
        latest = self.repo_path is not None or _is_latest(self.js_option, request)
...
class IndexView(HomeAssistantView):
    ...
    async def get(self, request, extra=None):
        ...
        return web.Response(
            text=template.render(**template_params), content_type="text/html"
        )
...
def _is_latest(js_option, request):
    ...
    if "latest" in request.query or (
        ...
        and "latest" in urlparse(request.headers["Referer"]).query
    ):
        return True
    ...
    if "es5" in request.query or (
        request.headers.get("Referer")
        and "es5" in urlparse(request.headers["Referer"]).query
    ):
        return False
...
def websocket_get_panels(hass, connection, msg):
    ...
    connection.to_write.put_nowait(websocket_api.result_message(msg["id"], panels))
