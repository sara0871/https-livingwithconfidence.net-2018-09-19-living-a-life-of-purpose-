...
PACKAGES_CONFIG_SCHEMA = vol.Schema(
    {
        cv.slug: vol.Schema(  # Package names are slugs
            {cv.slug: vol.Any(dict, list, None)}
        )  # Only slugs for component names
    }
)
...
def ensure_config_exists(
    config_dir: str, detect_location: bool = True
) -> Optional[str]:
...
def _format_config_error(ex: vol.Invalid, domain: str, config: Dict) -> str:
    ...
    if "extra keys not allowed" in ex.error_message:
        message += "[{}] is an invalid option for [{}]. Check: {}->{}.".format(
            ex.path[-1], domain, domain, "->".join(str(m) for m in ex.path)
        )
    else:
...
async def async_process_ha_core_config(
    ...
    has_trusted_networks: bool = False,
) -> None:
    ...
    if hac.elevation is None and hac.latitude is not None and hac.longitude is not None:
...
def async_process_component_config(
    ...
    if hasattr(component, "CONFIG_SCHEMA"):
        ...
        for p_name, p_config in config_per_platform(config, domain):
            ...
            try:
                p_validated = component.PLATFORM_SCHEMA(p_config)  # type: ignore
            except vol.Invalid as ex:
                async_log_exception(ex, domain, config, hass)
                continue
            ...
            if hasattr(platform, "PLATFORM_SCHEMA"):
                ...
                try:
                    p_validated = platform.PLATFORM_SCHEMA(p_validated)  # type: ignore
                except vol.Invalid as ex:
                    async_log_exception(
                        ex, "{}.{}".format(domain, p_name), p_validated, hass
                    )
                    continue
...
async def async_check_ha_config_file(hass: HomeAssistant) -> Optional[str]:
    ...
    res = await hass.async_add_executor_job(check_ha_config_file, hass)
