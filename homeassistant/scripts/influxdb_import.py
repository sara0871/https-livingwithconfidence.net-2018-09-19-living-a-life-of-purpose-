...
def run(script_args: List) -> int:
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help=("Do not write points but simulate preprocessing and print " "statistics"),
    )
    ...
    if not os.path.exists(src_db) and not args.uri:
        print(
            "Fatal Error: Database '{}' does not exist "
    ...
    while True:
        ...
        query = (
            ...
            .filter(models.Events.event_type == "state_changed")
            .order_by(models.Events.time_fired)
            .slice(step_start, step_stop)
        )
        ...
        for event in query:
            ...
            if (
                not ("entity_id" in event_data)
                or (excl_entities and event_data["entity_id"] in excl_entities)
                or (
                    excl_domains
